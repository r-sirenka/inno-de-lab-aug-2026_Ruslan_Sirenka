...
def calculate_overdue_fine(
    ...
    try:
        ...
        #единственный способ, чтобы строка --- Проверка транзакции возврата завершена ---
        #не выскакивала до фильма matrix это добавить её в блок try
        #но это не соответсвует требованиям блока try как я понял
